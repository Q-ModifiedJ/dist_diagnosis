...
def collect_cpu_saturation_data_twonode(number, node1, node2):
    assert node1 in [0, 1, 2, 3, 4]
    assert node2 in [0, 1, 2, 3, 4]
...
def collect_io_and_cpu_saturation_data_onenode(number, node):
    ...
    ob_controller = OceanBaseController()
    ob_monitor = OBMonitor(30, 5, labels=labels,extra_label=extral_label,server_num=5)
...
def collect_cpu_saturation_and_heavyload_data_onenode(number, node):
    assert node in [0, 1, 2, 3, 4]
    labels = ["load", "load", "load", "load","load"]
    extra_label = "node{}.cpu".format(node + 1)

    ob_controller = OceanBaseController()
    ob_monitor = OBMonitor(30, 5, labels=labels, extra_label=extra_label,server_num=5)
    cpu_abnormal = CPUSaturation(node=nodes[node])
...
def cpu():
    ...
    for i in range(9, 11):
        collect_cpu_saturation_data_onenode(i, 4)
        time.sleep(20)
...
def cpu_2():
    ...
    print("collecting cpu node0")
    collect_cpu_saturation_data_twonode(3, 0, 3)
    time.sleep(20)

    print("collecting cpu node0")
    collect_cpu_saturation_data_twonode(4, 0, 4)
    time.sleep(20)
...
def cpu_io():
    ...
    print("collecting cpu node4")
    for i in range(9, 11):
        collect_io_and_cpu_saturation_data_onenode(i, 4)
        time.sleep(20)
...
def cpu_and_load():
    ...
    print("collecting cpu node4")
    for i in range(9, 11):
        collect_cpu_saturation_and_heavyload_data_onenode(i, 4)
        time.sleep(20)
