...
# datasets = ["idle"]
# classes = ["idle", "normal"]
# dimensions = [] # 预计算好的过滤后保留的指标index，里面没有的指标就去除 (弃用)
...
def read_case(anomaly_case):
    # 首先获取当前文件夹中有几个case
    labels = [] # 三个Node，分开存   [case1labels,case2label] case1label: {node1:[labels],node1:[labels],node1:[labels]}
    data = []  # 三个Node，分开存    data = [[node1_data, node2_data, node3_data] for case1]

    anomaly_case = os.path.join("data", anomaly_case)
...
def prepare_data_for_distdiagnosis(datasets, classes):
    all_training_data = []
    all_training_labels = []
    all_node_metric = []
    for dataset in datasets:
        data, labels = read_case(dataset)
        # distdiagnosis训练数据，分三个node，每个case的data直接展平，打上
        ...
        for i in range(len(data)):
            ...
            training_data.append([node1data, node2data, node3data])
            training_labels.append([node1label, node2label, node3label])
...
def prepare_data_for_baseline(datasets, classes):

    all_training_labels = []
    all_training_data = []
    for dataset in datasets:
        data, labels = read_case(dataset)
        # xgboost等的训练数据，直接三个node的数据flatten后拼在一起
        # 标签也拼在一起
        ...
        for i in range(len(data)):
            ...
            training_data.append(np.concatenate((node1data, node2data, node3data), axis=0))
            training_labels.append([*node1label, *node2label, *node3label])
...
if __name__ == "__main__":
    data = read_ob_report_csv("data/buffer/case1/ob_report1.csv")
    print(data)
